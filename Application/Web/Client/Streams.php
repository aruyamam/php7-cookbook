...
class Streams
{
         ...
         case Request::METHOD_POST:
            $opts = [
               $request->getTransport() => [
                  ...
                  'header' => Request::HEADER_CONTENT_TYPE . ': ' . Request::CONENT_TYPE_FROM_URL_ENCODED,
                  'content' => $data
               ]
            ];
   ...
   protected static function getResult(Received $received, $resource)
   {
      // process results
      ...
      if ($received->getHeaderByKey(Received::HEADER_CONTENT_TYPE)) {
         switch (TRUE) {
            case stripos(
                     $received->getHeaderByKey(Received::HEADER_CONTENT_TYPE),
